// ═══ 银行直连交易标注面板 · /banking 与 /accounting 共用 ═══
// 依赖宿主页面提供: annApi(path,opts) 请求助手(banking 风格路径, 如 '/plaid/...')、
// esc()、money()、TOKEN、ROLE、loadTxns()、全局 _TXNS, 以及 annDrawer/annBackdrop/annToast/annLightbox 标记。
...
// 选项统一走 /plaid/ann-options (admin/客服/会计都可读, 之前调 /admin/* 会让客服和会计拿到空列表)
let _bsOptsLoaded = false;
async function _bsEnsureOptions() {
  if (_bsOptsLoaded) return;
  try {
    const r = await annApi('/plaid/ann-options');
    const srt = a => (Array.isArray(a) ? a.filter(Boolean) : []).sort((x, y) => x.localeCompare(y));
...
async function bsEnsureReferrers() { return _bsEnsureOptions(); }
async function bsEnsureWarehouses() { return _bsEnsureOptions(); }
async function bsEnsureCompanies() { return _bsEnsureOptions(); }
...
async function bsUpdateBoxField(id, field, value) {
  ...
  box[field] = value;
  try {
    const r = await annApi(`/plaid/annotations/${id}`, { method: 'PUT', body: JSON.stringify({ [field]: value }) });
    if (r && r.ann_status !== undefined) { box.ann_status = r.ann_status; box.ann_by = r.ann_by || box.ann_by; annStatusLineRefresh(box); }
...
function _bsAnnCheckHtml(box, items, mode) {
  ...
  const lines = items.map(it => {
    ...
    // 号后面标明数据来源, 金额对不上时一眼能看出比的是哪边的数。
    const srcTag = `<span style="color:#94a3b8;font-size:.66rem;font-weight:400">（${info.source === 'bintique' ? 'Bintique账单' : '本系统发票'}）</span>`;
    const numHtml = (info.source === 'bintique'
      ...
      : info.id ? `<a href="#" onclick="event.preventDefault();annShowInvoice(${info.id})" title="点击查看这张发票的明细" style="color:inherit;font-weight:700;text-decoration:underline">${esc(num)}</a>` : esc(num)) + srcTag;
    const probs = [], oks = [];
    // 账期: 两边都填了才比
...
async function _bsInvItemsSave(box, items) {
  ...
  _bsInvSaveTimers[box.id] = setTimeout(async () => {
    try {
      const r = await annApi(`/plaid/annotations/${box.id}`, { method: 'PUT', body: JSON.stringify({
...
async function bsBoxUploadPhotos(id, files) {
  ...
  try {
    const res = await fetch(API + `/plaid/annotations/${id}/photos`, { method: 'POST', headers: { Authorization: 'Bearer ' + TOKEN }, body: fd });
    const r = await res.json();
    if (!r.success) throw new Error(r.error || '上传失败');
    box.photos_urls = r.photos_urls || [];
    if (r.ann_status) { box.ann_status = r.ann_status; annRefreshChip(box); }
    _bsRenderBoxPanel();
    showToast('照片已上传 ✓');
...
async function bsBoxDeletePhoto(id, url) {
  ...
  const box = _bsBoxList.find(b => b.id === id); if (!box) return;
  try {
    const r = await annApi(`/plaid/annotations/${id}/photos`, { method: 'DELETE', body: JSON.stringify({ photo: url }) });
...
async function annDeleteBox(id) {
  ...
  const box = _bsBoxList.find(b => b.id === id); if (!box) return;
  try { await annApi(`/plaid/annotations/${id}`, { method: 'DELETE' }); }
...
function annStatusLineRefresh(box) {
  const el = document.getElementById('annStatusLine');
  if (!el || !box || box.id !== _annOpenId) return;
  el.outerHTML = annStatusLineHtml(box);
}
  ...
  panel.innerHTML = annStatusLineHtml(box)
    + `<div class="bs-box-item" data-id="${box.id}" style="border:1px solid var(--gray-200);border-left:3px solid ${color};border-radius:8px;padding:.5rem .55rem;margin-bottom:.5rem">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.35rem">
        ...
        ${ROLE === 'admin' ? `<button onclick="annDeleteBox(${box.id})" title="删除" style="border:none;background:#fef2f2;color:#b91c1c;border-radius:4px;cursor:pointer;font-size:.8rem;padding:.1rem .45rem">🗑 删除</button>` : ''}
      </div>
      ...
      <div class="bs-acct-wrap" style="display:${box.payee === BS_PAYEE_ACCT ? '' : 'none'};background:#ecfdf5;border:1px dashed #a7f3d0;border-radius:6px;padding:.45rem .5rem;margin-bottom:.35rem">
...
// 「待审核」筛选按钮: 有待审核标注时出现, 显示条数; 点击只看这些交易
let _pendingOnly = false;
function updatePendBtn() {
  const btn = document.getElementById('pendBtn');
  if (!btn) return;
  const n = _bsBoxList.filter(b => b.ann_status === 'pending').length;
  if (!n) { btn.style.display = 'none'; if (_pendingOnly) { _pendingOnly = false; loadTxns(); } return; }
  btn.style.display = '';
  btn.textContent = (_pendingOnly ? '✕ 退出待审核视图（' : '⏳ 待审核 ') + (_pendingOnly ? n + '）' : n + ' 条');
  btn.style.background = _pendingOnly ? '#fff7ed' : '';
  btn.style.borderColor = _pendingOnly ? '#fed7aa' : '';
  btn.style.color = _pendingOnly ? '#c2410c' : '';
}
...
async function annOpen(txnId) {
  try {
    ...
    // 收支方向由交易金额定死 (Plaid 正数=支出)；旧标注若被改错，管理员打开时纠正回来
    if (t && ROLE === 'admin') {
      const expect = (t.amount || 0) > 0 ? 'out' : 'in';
      if (box.direction !== expect) {
        box.direction = expect;
        try { await annApi('/plaid/annotations/' + box.id, { method: 'PUT', body: JSON.stringify({ direction: expect }) }); } catch (e2) {}
      }
    }
