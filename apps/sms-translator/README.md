...
- **发短信**：在电脑上写，**每个会话可以单独选发送语言**（记住，下次还用这个）。
  选了语言之后主按钮就是「译成 XX 并预览」——先把 Claude 的译文显示出来，确认了
  再点「发送译文」，也可以「直接发原文」。
...
- **网页那边的会话工具**：对方语言 / 我的语言两个下拉、收到的消息自动翻译开关、
  一键「重新翻译」（电脑收到后重翻再推回来）、星标、标签、关闭会话、未读。
...
**最省事：插一次 USB 线，点设置里的「切换到无线（先插一次 USB）」，然后拔线。**
一个按钮搞定，不用在手机上找任何东西。

**老实说：无线就是不如插线稳。** 三星手机息屏一段时间后 WiFi 会打盹，无线连接就
断了——软件会在手机醒来后几秒内自动连回去，掉线期间收发都会排队不丢，但「一直
在线」这件事无线做不到。想彻底稳，两个办法：一直插着 USB 线（顺便充电）；或者
手机插着充电器 + 开发者选项里打开「充电时保持唤醒」，WiFi 就不会睡。

不想插线就走配对码那条路。要注意手机上的「无线调试」**要点这一行文字进去**，不是
只把右边的开关拨开；进去之后才有「使用配对码配对设备」。配对成功后软件会**自己
用 mDNS 找到连接地址并连上**——连接端口和配对端口不是同一个，这是以前最容易卡住
的地方，现在不用你去核对了。没自动连上就点「自动查找并连接」。
...
**网页上也能发短信**：那一页每个对话下面有输入框，写完点发送；也可以点右上角
「＋ 新短信」给一个**新号码**发。发之前可以选「译成 XX」——翻译在电脑这一侧做
（网站上没有 API key），电脑会先翻好再用手机发出去。备注名和备注两边是**同一份**，
在网页上改、在软件里改都会同步过去（谁改得晚算谁的）。网站自己发不出去
...
1. **换一根线**。充电线十有八九不带数据，或数据针脚接触不良——用原装线或明确标着
   「数据线」的线。这是第一大原因。
2. **插台式机后面的 USB 口**，别用集线器、延长线、机箱前面板的口。
3. **关掉 Windows 的 USB 节能**：设备管理器 → 通用串行总线控制器 → 每个「USB
   Root Hub」→ 电源管理 → 取消「允许计算机关闭此设备以节约电源」；再到 电源选项 →
   更改高级电源设置 → USB 设置 → 把「USB 选择性暂停」设为已禁用。
4. **卸载或退出三星 Smart Switch / Kies**。它们的后台服务会抢 USB 连接，和 adb
   打架，表现就是每隔一阵断一次。
5. 手机通知栏里把 USB 用途设为「文件传输」。

软件这边也加了一层：连接丢了会先自动重启 adb 服务再找一次（Windows 上 adb 服务
被别的程序抢线后会卡死，这是它的标准解法），救不活才报错。
...
### 4. 发出去的短信，手机不一定会写进短信库

三星在对方也支持时会走 **RCS（聊天消息）**，那种消息**不会写进手机的短信数据库**，
所以电脑这边读不到自己刚发出去的那条。表现是：气泡一直显示「已交给手机发送…」。
5 分钟后会改成「已发出 · 手机短信库里查不到」并保留在会话里——**不代表没发出去**，
对方通常已经收到了。要让它变成正常的短信，可以在手机短信 App 里关掉「聊天功能 /
RCS」。
...
翻译和发送前检查都要等 Claude 回话，Opus 在高峰期经常返回「忙不过来」(529)。
软件会自动重试几次，但等待感就是这么来的。两个开关：

- **设置 → 模型（收到的短信）**：换成 `claude-sonnet-5` 或 `claude-haiku-4-5`，
  短信这种一两句话的内容，Haiku 完全够用，快很多也便宜很多。
- **设置 → 模型（发送前）**：默认就是 Haiku —— 这两件事你是站着等结果的。
